...
data_overlap_waterlevel <- data %>% 
  mutate(waterlevel_round = plyr::round_any(waterlevel_start.x, 20),
         tide_n_round = plyr::round_any(tide_number.x, 15)) %>% 
  count(waterlevel_round, tide_n_round)
...
ggsave(filename = "figs/fig_overlaps_waterlevel_tide.png",
       dpi = 300)
```
