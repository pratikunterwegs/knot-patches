...
output_data <- map(data_files, function(df){
  ...
  temp_data <- funcInferResidence(somedata = temp_data,
                                  infResTime = 2,
                                  infPatchTimeDiff = 30,
                                  infPatchSpatDiff = 100)
  
  # watlasUtils function to classify path
  temp_data <- funcClassifyPath(somedata = temp_data,
                                resTimeLimit = 2)
  
  # watlasUtils function to get patches
  patch_data <- funcGetResPatch(somedata = temp_data,
                                bufferSize = 10,
                                spatIndepLim = 100,
                                tempIndepLim = 30,
                                restIndepLim = 30,
  ...
  patch_data <- funcGetPatchData(resPatchData = patch_data,
                                 dataColumn = "data",
                                 whichData = "spatial")
