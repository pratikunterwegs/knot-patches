...
tides <- VulnToolkit::HL(waterlevel$level, waterlevel$dateTime, period = 12.41, tides = "H", semidiurnal = TRUE)
...
map(data_files, function(df){
  ...
  temp_data <- merge(temp_data, tides, by.x = "ts", by.y = "time", 
                     all = TRUE)
  
  setorder(temp_data, ts)
  
  temp_data <- temp_data[,tide := !is.na(tide) 
         ][, tidalCycle := cumsum(tide) 
           ][,tidalTime := as.numeric(difftime(ts, ts[1], units = "mins")), 
             by = tidalCycle 
             ][complete.cases(X),]
              
  temp_data[,`:=`(tide = NULL, level = NULL, time = NULL)
         ][,tidalCycle := tidalCycle - min(tidalCycle) + 1
           ][,tidalCycle := str_pad(tidalCycle, 3, pad = 0)]

  fwrite(temp_data, file = df, dateTimeAs = "ISO")
  print(glue('tag {unique(temp_data$TAG)} added time since high tide'))
