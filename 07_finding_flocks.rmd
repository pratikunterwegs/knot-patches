...
data_id <- mutate(data_id,
                  behav = Mean) %>% 
  # drop_na(behav) %>% 
  select(id, mass, gizzard_mass, behav, month)
```
...
# remove bad data
flocks <- drop_na(flocks, month, trait)

# get within flock variance in traits
flock_variances <- flocks %>% 
  group_by(flock, trait) %>% 
  summarise(trait_variance = var(trait_value, na.rm = TRUE))
```
...
flock_covariates <- flock_covariates %>% 
  mutate(tide_stage = if_else(waterlevel_mean < 25, "low", "high"),
...
ggplot(flock_variances) +
  stat_bin_2d(aes(tidaltime_mean, trait_variance),
           geom = "tile") +
  scale_fill_viridis_c(limits = c(0, 100),
                       na.value = "yellow") +
  coord_fixed(ratio = 1600)+
  facet_grid( ~ trait, switch = "both")
```

```{r}
ggplot(flock_variances %>% 
         drop_na(trait_variance))+
...
ggsave("figs/fig_within_flock_variance.png",
       dpi = 300)
```
