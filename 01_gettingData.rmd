...
```{r install_watlasutils, message=FALSE, warning=FALSE}
...
```{r get_data, eval=FALSE, echo=TRUE, message=FALSE, warning=FALSE, include=FALSE}
# create a data storage file if not present
# use the getData function from watlasUtils on the tag_info data frame
...
pmap(tag_info[,.(Toa_Tag, Release_Date)], function(Toa_Tag, Release_Date){
  ...
  prelim_data <- watlasUtils::funcGetData(tag = Toa_Tag,
  ...
  print(glue('tag {Toa_Tag} accessed with {nrow(prelim_data)} fixes'))
  
  fwrite(prelim_data, file = glue('data/data2018/{Toa_Tag}.csv'))
