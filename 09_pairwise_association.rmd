...
data_summary <- data %>%
  ...
  summarise(n_associations = length(.),
...
model_n_assoc_reduced <- glmer(n_associations ~ 
                                 diff_behav * tide_stage +
                                 (1|tide_number) +
                                 (1|uid_pair),
                               data = data_subsample,
...
summary(model_n_assoc_reduced)
```

It seems that differences in behaviour are sufficient to explain the observed number of overlaps. Examine the AIC scores.

```{r}
# get aic difference
AIC(model_n_assoc_full) - AIC(model_n_assoc_reduced)

# get R2
MuMIn::r.squaredGLMM(model_n_assoc_reduced)
```
...
```{r}
model_output = capture.output(summary(model_n_assoc_reduced))
...
# run an alternative model with only behaviour
model_sp_overlap_reduced <- lmer(tot_overlap_space ~ 
                         diff_behav * tide_stage +
                         (1|tide_number) +
                         (1|uid_pair),
                       data = data_subsample)
...
summary(model_sp_overlap_reduced)
```

It seems that differences in behaviour are sufficient in this case also to explain the observed number of overlaps. Examine the AIC scores.

```{r}
# get aic difference
AIC(model_sp_overlap_full) - AIC(model_sp_overlap_reduced)

# get R2
MuMIn::r.squaredGLMM(model_sp_overlap_reduced)
```

```{r}
model_output = capture.output(summary(model_sp_overlap_reduced))
...
# run an alternative model with only behaviour
model_time_overlap_reduced <- lmer(tot_overlap_time ~ 
                         diff_behav * tide_stage +
                         (1|tide_number) +
                         (1|uid_pair),
                       data = data_subsample)
