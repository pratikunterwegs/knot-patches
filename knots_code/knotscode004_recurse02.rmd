...
load("knots01_recurse.rdata")
```

```{r get_recurses}
#'list files
recurse_files = list.files(pattern = "knots_revisit_", full.names = T)
...
k1.recurse = k1.recurse.test %>% mutate(time.num = as.numeric(time)) %>% left_join(recurses %>% mutate(id = as.character(id)), by = c("time.num" = "time", "id"))
