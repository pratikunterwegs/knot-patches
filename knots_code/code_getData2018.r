...
for(i in 1:length(tags)) {
 #'get knots 1
  ...
  if(i %% 10 == 0) {save(data2018.raw, file = paste("knots2018raw",i-10,"to",i,".rdata",sep = "")); rm(data2018.raw);gc();data2018.raw = list()}

  if(i == length(tags)) {data2018.raw <- data2018.raw[unlist(lapply(data2018.raw, class))=="list"]; save(data2018.raw, file = paste("knots2018rawfinal",".rdata",sep = "")); rm(data2018.raw);gc();data2018.raw = list()}
}

#'load all the rdata objects and make one df
data.files <- list.files(path = ".", pattern = "knots2018raw")
