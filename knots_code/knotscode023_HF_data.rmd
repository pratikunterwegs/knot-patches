...
data.tide <- data.prep %>% 
  map(function(x){
    x %>% mutate(ht = as.numeric(ht)) %>%
      right_join(tide.mins %>% filter(ht == as.numeric(x$ht)), by = c("time","ht"))
  })
...
data.tide <- rbind_list(data.tide) %>% 
  group_by(ht) %>% 
  mutate(ht.time = as.numeric(time) - as.numeric(min(time))) %>% 
  ungroup() %>% 
...
data.lt <- data.tide %>% 
  map(function(x){
    x <- x %>% filter(ht.time %between% c(3.5*3600,9.5*3600))
  })
...
coherence.scores <- proximity100 %>% 
  ...
  left_join(coherence.sim %>% mutate(ht = as.numeric(ht))) %>% 
