...
for(i in 1:length(focals)){
  ...
  for(j in 1:length(nonfocals)){
    distances <- spDists(as.matrix(focals[[i]][,c("x","y")]),
                          as.matrix(nonfocals[[j]][,c("x","y")]), diagonal = T)
    #'get only distances where both focal and nonfocal positions are known
    distances <- distances[focals[[1]]$x < 9e9 & nonfocals[[1]]$x < 9e9]
    dist_class <- data_frame(distances = sum(distances <= 25)) %>%
      #count(class) %>% 
    ...
    proximity <- rbind(proximity, dist_class)
    }
  }
}
```
