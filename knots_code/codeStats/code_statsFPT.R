...
dataTagWeek = data %>% 
  group_by(id) %>% 
  summarise(tagWeek = lubridate::week(first(Release_Date)))
...
ggplot(dataRevDay)+
  geom_jitter(aes(day2, value), shape = 1)+
  facet_grid(variable~tagWeek, scales = "free_y")+
  themePubLeg()+
  xlab("# tidal cycle")+ ylab("# visits / minutes / minutes")+
  ggtitle("Space use metrics ~ time in 8 tidal cycle bins")
...
dataRevDay %>% 
  ...
  labs(x = "first posn. time - release time (hrs)",
       ...
       title = "Fixed release-lag effect on FPT")+
  coord_fixed(ratio = 5)
