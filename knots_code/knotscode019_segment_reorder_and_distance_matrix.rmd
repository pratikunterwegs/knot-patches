...
library(ggplot2)
#'plot example
x %>% dplyr::select(x,y,state,new.seg) %>%
  melt(id.vars = c("x","y")) %>%
  ggplot()+
...
for(i in 1:length(a)){
  ...
  for(j in 1:length(b)){
    ...
    #### this is the new method of looking for overlap ####
    #'check if the focal segment start time is less than non-focal end time, and if focal end time is greater than non-focal start time
    
    time.matrix1 = outer(a[[i]]$time.start, b[[j]]$time.end, FUN = "<")
    time.matrix2 = outer(a[[i]]$time.end, b[[j]]$time.start, FUN = ">")
    
    time.matrix = (time.matrix1 + time.matrix2)
    
    #temp.prox = apply(time.matrix, 2, function(x) abs(x) <= a[[i]]$seg.dur/2)

    #'if the distance between 2 segments is too great in EITHER space OR time, assign a 0
    dist.matrix[dist.matrix >= 250] = 0
    dist.matrix[time.matrix < 2] = 0
    
    #'find the sum of events in which the focal and non-focal birds were BOTH temporally
    #'AND spatially proximate
    seg.prox.count = apply(dist.matrix, 1, function(x) sum(x > 0))
