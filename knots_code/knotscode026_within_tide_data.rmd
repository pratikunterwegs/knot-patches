...
ggplot()+
  geom_jitter(data = proximity_aj,
              aes(x = hour, y = c_emp), shape = 1,
              col = "grey80")+
  geom_pointrange(data = cij_5_top,
              aes(x = hour, y = c_empm, ymin = c_empm-ci,ymax = c_empm+ci,
                  colour = "Mean 10 highest \n coherences per id \n ± 95% CI"),
              size = 1)+
  scale_colour_manual(values = solcol[4])+
...
cij.state <- filter(proximity_aj, !hour %between% c(4:8)) %>% mutate(state = ifelse(hour < 4, "receding","advancing")) %>% group_by(pair, state, ht) %>% summarise(c = mean(c_emp, na.rm = T)) %>% spread(state, c)
...
mod.c.cons.state <- glmmTMB(advancing ~ receding + (1|pair)+(1|ht), data = cij.state, family = binomial, ziformula = ~1)
```
