...
lm.data <- coherence.correlation %>% 
  ...
  ungroup() %>% 
  mutate(dist_diff = sqrt(abs(distance_sum.x - distance_sum.y)),
         speed_diff = log(abs(speed_exp.x - speed_exp.y))+9,
         fpt_diff = log(abs(fpt_exp.x - fpt_exp.y))+8) %>% 
  select(focal, nonfocal, ht.time, dist_diff, speed_diff, fpt_diff, interactions, coherence_emp, c1, ht)
```

```{r}
library(lme4)
#'run glmm
mod.coherence.consistency = lmer(coherence_emp ~ 
                                   c1 + 
                                   ht.time + 
                                   dist_diff +
                                   fpt_diff + 
