...
```{r}
yorno = brewer.pal(3, "Set2")[c(1,2)]
#'plot which birds are okay to use
pdf(file = "knots_figs/id_tide_sampling.pdf", height = 8, width = 11)

ggplot(metrics)+
  geom_tile(aes(x = id, y = ht, fill = prop), col = "grey50", lwd = 0.1)+
  geom_tile(data = a, aes(x = variable, y = ht, fill = value), fill = ifelse(a$value == T, yorno[1], yorno[2]), col = "grey50", lwd = 0.1)+
  geom_point(data = a, aes(x = variable, y = ht, group = variable, shape = value), size = 1)+
  geom_text(data = metrics, aes(x = id, y = ht, label = round_any(prop, 0.1), col = prop > 0.3), hjust = "middle", size = 2)+
  scale_y_reverse(breaks = c(1:max(as.numeric(metrics$ht))))+
  scale_x_discrete(position = "top")+
  scale_fill_viridis()+
  scale_colour_manual(values = c("white",1), guide = F)+
  scale_shape_manual(values = c(4, 1))+
  theme_bw()+
  theme(legend.position = "top", panel.grid= element_blank(), panel.grid.minor = element_blank())+
  labs(list(y = "high tide #", fill = "proportion sampled",
            shape = "inclue tide?"))
...
k1.recurse = getRecursions(k1.prep2[[i]][c("x","y","time","id")], radius = 250, timeunits = "hours")
