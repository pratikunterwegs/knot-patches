...
ggplot(pairs_test)+
  geom_tile(aes(x = focal, y = nonfocal), fill = "white",col = "white", size = 1)+
  geom_tile(aes(x = focal, y = nonfocal, fill = c_emp, alpha = p.value <= 0.05), col = "white", size = 1)+
  scale_fill_viridis(option = "plasma")+
  scale_alpha_manual(values = c(0.1,1), guide = F)+
  theme_poster()+
  theme(axis.text = element_text(size = 8, angle = 40),
        legend.text = element_text(size = 12, angle = 30),
        axis.ticks = element_blank(),
        panel.grid.major = element_line(colour = "grey90"),
        legend.title = element_text(size = 14))+
  labs(list(fill = "Pair-wise coherence"))
...
ggplot()+
 # stat_density(data = proximity, aes(x = coherence_emp, col = as.factor(pair)), geom = "line", position = "identity")+
  theme_poster()+
  stat_density(data = proximity, aes(x = coherence_emp), geom = "line", size = 2, col = solcol[1])+
  theme(legend.position = "none")+
  labs(list(x = "Pairwise coherence", y = "Density"))
...
count(pairs_test, friends = c_emp > c_sim, sig = p.value <=0.05) %>% mutate(prop = n/(sum(n))) %>%
  ggplot()+
  geom_tile(aes(x = friends, y = sig, fill = prop), col = "white", size = 2)+
  geom_text(aes(x = friends, y = sig, label = paste(round_any(prop*100,0.1), "%")), col = c(1,"white",1,"white"), size = 12)+
  #scale_fill_viridis()+
  theme_poster()+
  theme(legend.position = "none")+
  labs(list(x = "Pairwise coherence", y = "K-S test significance \n P-value ≤ 0.05"))
