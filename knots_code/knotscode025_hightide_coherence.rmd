...
for(i in 1:length(coherence.ht)){
  ...
  colnames(coherence.ht[[i]]) <- c(colnames(a), "focal", "hour")
}
...
coherence.scores.ht <- proximity100.ht %>% 
  ...
  left_join(coherence.sim.ht) %>% 
  mutate(diff = coherence_emp - coherence_sim)

#'retain only pairs
coherence.scores.ht <- coherence.scores.ht %>% left_join(pairs) %>% filter(!is.na(pair))
```
...
coherence.test <- coherence.test.ht %>% 
