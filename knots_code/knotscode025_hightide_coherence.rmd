...
cairo_ps(filename = "figure_coherence_test.eps", width = 150/25.4, height = 150/25.4)
...
ggplot()+
  ...
  geom_tile(data = pairs_test, aes(x = as.numeric(as.factor(focal)), y = as.numeric(as.factor(nonfocal)), fill = c_emp), col = "white", size = 1)+
  geom_tile(data = pairs_test, aes(x = as.numeric(as.factor(focal)), y = as.numeric(as.factor(nonfocal)), fill = c_emp, alpha = p.value <= 0.05), col = "white", size = 1)+
