...
ggplot(pairs_test)+
  ...
  labs(list(fill = "Pair-wise co-occurrence"))
...
ggplot()+
  ...
  theme_poster()+
  geom_histogram(data = proximity, aes(x = coherence_emp, y = ..count../sum(..count..)), geom = "line", fill = colb1, col = solcol[3])+
 # scale_fill_manual(values = brewer.pal(9,"YlOrBr")[5])+
  theme(legend.position = "none")+
  #coord_cartesian(ylim = c(0,0.1))+
  labs(list(x = "Pairwise co-occurrence", y = "Proportion"))
...
count(pairs_test, friends = c_emp > c_sim, sig = p.value <=0.05) %>% mutate(prop = n/(sum(n))) %>%
  ...
  geom_text(aes(x = friends, y = sig, label = paste(round_any(prop*100,0.1), "%")), col ="white", size = 12)+
  scale_fill_gradientn(colors = (brewer.pal(4,"RdGy")))+
  theme_poster()+
  theme(legend.position = "none")+
