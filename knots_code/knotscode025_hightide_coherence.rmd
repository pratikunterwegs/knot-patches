...
proximity100.ht <- proximity %>%
  ...
  summarise(pos_focal = first(pos_focal),
    ...
    T ~ interactions/(first(pos_focal) + first(pos_nonfocal) - interactions)))
})
```
...
coherence.test.ht <- coherence.scores.ht %>% 
  group_by(focal, nonfocal, ht.hr) %>% mutate(n = length(!is.na(coherence_emp))) %>% 
  filter(n >= 5) %>%
