...
ggplot()+
  ...
  geom_text(aes(x = 4e3,y=60, label = "GLMM \n z-statistic = -3.85 \n P-value < 0.001"), size = 10, vjust = "inward")+
  theme_pub()+
  theme(axis.text = element_text(size = 16), axis.title = element_text(size = 20))+
...
coherence.test %>% mutate(cdiff = mean_coherence_emp - mean_coherence_sim) %>% filter(!is.na(cdiff)) %>% 
  ...
  geom_text(aes(x = friends, y = p, label = paste(round_any(prop*100,0.1), "%")), col = c(1,1,1,"white"), size = 12)+
  ...
  labs(list(x = "Coherence", y = "P-value ≤ 0.05"))
