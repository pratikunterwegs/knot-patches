...
#'prg: why 0.7? should this come from the data? basically, the correlation between steps at t and t+1

StpSize_ind=10 

#Mean step lengths of individuals;

#'prg: in what units? assuming metres
...
# plotting step size distribution: check to see what step length distribution has been produced
...
#'set social perception range to 0 in the paired agents scenario, as agents only follow their 'mates'

N_tmStp=DaysToSimulate*DayLength 

#' total number of time steps to simulate per iteration, basically total minutes in the sim

N_Rows=N_indv*N_tmStp

#'prg: set the number of rows in the df as the total minutes times number of indivs
```

```{r make_dfs}

#### NOTE: MAKE THIS IN THE TIDYVERSE ####

#'prg: create a dataframe to hold the eventual positions per simulation run: the individual identity, time step or position id overall, day-wise position id, unique secondary time step id (ie, not repeated among individuals)

XYind_log2 <- data_frame(
  ...
  burst = as.factor(rep(seq(1:(DaysToSimulate*N_indv)),length.out=N_Rows,each=DayLength)),
  x = NA,
  y = NA,
  pseudosex = NA)
```
...
#Determining indivduals to drift by sampling randomisation
...
for (Curr_tmStp in 1:(N_tmStp-1)){
  ...
  Dist=rep(NA,N_indv); 
  for (ii in 1:N_indv) {
    Dist[ii] = dist(rbind(XYind[[ii]][Curr_tmStp, ],c(XYind[[Curr_indv]][Curr_tmStp, ])))}
...
Name1=paste("spiegel_sim_01", N_tmStp , N_indv,  100*EtaCRW, StpSize_ind,DriftHRCenters,".rdata",sep="_")
