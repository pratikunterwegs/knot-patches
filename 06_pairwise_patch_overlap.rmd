...
## Total overlap between individuals

How much to pairs of individuals overlap?

```{r}
# get id wise overlap and plot
data_id_overlap <- data_overlap[,.(total_time_overlap = sum(temporal_overlap_seconds),
                           total_area_overlap = sum(spatial_overlap_area)),
                        by = c("id.x", "id.y")]

# export
fwrite(data_id_overlap, file = "data/data2018/data_id_overlap_2018.csv")

# prepare to plot
data_id_overlap <-  
  setDF(data_id_overlap) %>% 
  as_tibble()

# nest data
data_id_overlap %<>%
  pivot_longer(cols = c("total_time_overlap",
                        "total_area_overlap")) %>% 
  group_by(name) %>% 
  nest()

# plot as heatmap with decent units
fig_list <- pmap(data_id_overlap, function(name, data){
  ggplot(data)+
  geom_tile(aes(factor(id.x), factor(id.y), 
                fill = value))+
  scale_fill_viridis_c()+
  theme_dark(base_size = 6)+
    scale_x_discrete(drop=T)+
    scale_y_discrete()+
    labs(x = "Focal individual",
         y = "Paired individual",
         title = name)+
