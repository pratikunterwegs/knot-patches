...
map(data_files, function(df){
  ...
  map(temp_data, function(tempdf){
    ...
    df_recurse <- getRecursions(x = temp_data[,c("X","Y","ts","id")], 
                                radius = radius, 
    ...
    fwrite(temp_df, file = glue('data/data2018/revisitData/{unique(temp_df$id)}_{unique(temp_df$tidalCycle)}_revisit.csv'))
    
    print(glue('recurse {unique(temp_df$id)}_{unique(temp_df$tidalCycle)} done'))
